...
final public class APIClient {
    ...
    ///   - jwtToken: An optional String to indicate if request should be performed with JWT Authorization or not
    ///   - completion: Completion handler to call when request is completed.
    private func sendRequest<T: APIRequestWithExplicitBody>(_ request: T) async -> Result<(T.Response, Int), APIClientError> {
...
//        }
    ...
    public func send<T: APIRequestWithExplicitBody>(_ request: T) async -> Result<(T.Response, Int), APIClientError> {
...
//        }
    ...
    /// Create finalURL
    private func endpoint<T: APIRequestWithExplicitBody>(for request: T) -> URL {
        guard let baseUrl = URL(string: request.resourceName, relativeTo: baseUrl) else {
            fatalError("Bad resourceName: \(request.resourceName)")
        }
        return baseUrl
    ...
    /// Create urlencoded body
    private func body<T: APIRequestWithExplicitBody>(for request: T) -> Data? {
        var bodyData: Data
        let encoder = JSONEncoder()
        do {
            bodyData = try encoder.encode(request.body)
        } catch {
            log.error("Couldn't encode HTTPRequest.body. Body is nil for non-GET request")
            return nil
        }
